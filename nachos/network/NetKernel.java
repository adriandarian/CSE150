...
public class NetKernel extends VMKernel {
  ...
  /**
   * Test the network. Create a server thread that listens for pings on port 1 and
   * sends replies. Then ping one or two hosts. Note that this test assumes that
   * the network is reliable (i.e. that the network's reliability is 1.0).
   */
  ...
  private void pingServer() {
    while (true) {
      ...
      try {
        ack = new MailMessage(ping.packet.srcLink, ping.srcPort, ping.packet.dstLink, ping.dstPort, ping.contents);
      } catch (MalformedPacketException e) {
        // should never happen...
        continue;
      }
